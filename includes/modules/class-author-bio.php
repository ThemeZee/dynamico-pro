...
class Dynamico_Pro_Author_Bio {
	...
	static function setup() {
		...
		add_action( 'dynamico_author_bio', array( __CLASS__, 'author_bio' ) );
			...
			<div class="entry-author clearfix">
				...
				<div class="author-info">
					...
					<div class="author-heading">
						<h4 class="author-title entry-title"><?php printf( esc_html__( 'About %s', 'dynamico-pro' ), '<span class="author-name">' . get_the_author() . '</span>' ); ?></h4>
						<a class="author-link" href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>" rel="author">
							<?php esc_html_e( 'View all posts', 'dynamico-pro' ); ?>
						</a>
					</div>
	...
	static function author_bio_settings( $wp_customize ) {
		...
		$wp_customize->add_setting( 'dynamico_theme_options[author_bio]', array(
			'default'           => false,
			'type'              => 'option',
		...
		$wp_customize->add_control( 'dynamico_theme_options[author_bio]', array(
			'label'    => __( 'Display author bio', 'dynamico-pro' ),
...
// Run Class.
add_action( 'init', array( 'Dynamico_Pro_Author_Bio', 'setup' ) );
