 ...
 * License Key Class
 */
class Dynamico_Pro_License_Key {
	/**
	 * Init Function
	 *
	 * @return void
	...
	static function setup() {
		...
		require_once DYNAMICO_PRO_PLUGIN_DIR . 'includes/customizer/class-customize-license-control.php';
	...
	static function license_settings( $wp_customize ) {
		...
		$wp_customize->add_setting( 'dynamico_theme_options[license_key]', array(
			'default'           => '',
			'type'              => 'option',
		...
		$wp_customize->add_control( new Dynamico_Pro_Customize_License_Control(
			$wp_customize, 'dynamico_theme_options[license_key]', array(
				'label'    => esc_html__( 'License Key', 'dynamico-pro' ),
				'section'  => 'dynamico_section_theme_info',
				'settings' => 'dynamico_theme_options[license_key]',
				'priority' => 30,
			)
		) );
	}
	...
	static function activate_license() {
		...
		if ( '' === $license ) {
			...
			update_option( 'dynamico_theme_options', $theme_options );
			die();
		}
		...
		// Call the custom API.
		$response = wp_remote_post( DYNAMICO_PRO_STORE_API_URL, array(
			'timeout'   => 25,
			'sslverify' => true,
			'body'      => $api_params,
		) );
		...
		delete_transient( 'dynamico_license_check' );
	...
	static function check_license() {
		...
		$status = get_transient( 'dynamico_license_check' );
		...
		if ( false === $status ) {
			...
			set_transient( 'dynamico_license_check', $status, DAY_IN_SECONDS );
		}
